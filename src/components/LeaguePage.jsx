...
function getTeamTopScorer(team, playerGoals) {
  let best = null;
  team.players.forEach((p) => {
    const goals = playerGoals[p.id] || 0;
...
function useSelectedTeam(season) {
  ...
  const topScorer = getTeamTopScorer(standing.team, season.playerGoals);
...
function TeamProfileRoute({ season }) {
  ...
  return (
    <TeamProfileView
      ...
      recentForm={selected.recentForm}
      topScorer={selected.topScorer}
      playerGoals={season.playerGoals}
...
function LeaguePage() {
  ...
  const { teams: savedTeams, loading: teamsLoading } = usePublishedTeams(selectedSeasonId);
      ...
      <div className="generator-content">
        ...
        <Routes>
          ...
          <Route
            ...
            element={<TeamProfileRoute season={season} />}
          />
        </Routes>
      </div>
    </>
